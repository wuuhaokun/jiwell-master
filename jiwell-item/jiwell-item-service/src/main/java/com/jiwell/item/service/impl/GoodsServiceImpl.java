...
import com.jiwell.item.bo.SeckillParameter;
import com.jiwell.parameter.pojo.SpuQueryByPageParameter;
import com.jiwell.item.bo.SpuBo;
import com.jiwell.item.mapper.*;
import com.jiwell.item.pojo.*;
import com.jiwell.item.service.CategoryService;
import com.jiwell.item.service.GoodsService;
...
public class GoodsServiceImpl implements GoodsService {
    ...
    public PageResult<SpuBo> querySpuByPageAndSort(SpuQueryByPageParameter spuQueryByPageParameter) {
        ...
        //3.3 是否檢查符合cid3
        if (spuQueryByPageParameter.getCid3() != -1) {
...
//            spuBo.setSkus(skuList);
        ...
        return new PageResult<>(pageInfo.getTotal(),totalPage,list);
    }
    ...
    public Sku querySkuById(Long id) {
        Example temp = new Example(Stock.class);
        temp.createCriteria().andEqualTo("skuId", id);
        Stock stock = this.stockMapper.selectByExample(temp).get(0);
